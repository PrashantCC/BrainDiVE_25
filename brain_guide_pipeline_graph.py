...
class mypipelineSAG(DiffusionPipeline):
    ...
    def cond_fn(self,
        ...
        comb_mean_act, _, _ = self.brain_tweak(normalized_image)
        comb_mean_act = comb_mean_act * clip_guidance_scale
       
        # grads = -torch.autograd.grad(loss, latents)[0]
        # print(f"loss_type = {type(loss)}, loss_dtype = {loss.dtype}, loss_shape = {loss.shape}, loss_requires_grad = {loss.requires_grad}, loss_grad_fn = {loss.grad_fn}")
        # print(f"latents_type = {type(latents)}, latents_dtype = {latents.dtype}, latents_shape = {latents.shape}, latents_requires_grad = {latents.requires_grad}, latents_grad_fn = {latents.grad_fn}")
        
        grads = torch.autograd.grad(comb_mean_act, latents)[0]
    ...
    def __call__(
        ...
        for i, t in enumerate(timesteps):
            ...
            comb_mean_act, mean_act1, mean_act2 = self.brain_tweak(normalized_image)
        
            comb_act_vector.append(comb_mean_act.item())
            act_vector_1.append(mean_act1.item())
            act_vector_2.append(mean_act2.item())
        ...
        return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), (comb_act_vector, act_vector_1, act_vector_2)
