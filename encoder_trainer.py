...
# import model_vit
import encoder_model_vit_update # I made this update   # **
...
def train_net(rank, world_size, freeport, other_args):
    ...
    s, functional, region, = other_args.subject_id, other_args.functional, other_args.region
    gpu_id = other_args.gpu_id
    ...
    output_device = rank + gpu_id
    dist.init_process_group(backend="nccl", rank=rank, world_size=world_size)
    torch.cuda.set_device(rank + gpu_id)
    torch.backends.cudnn.benchmark = True
    ...
    feature_extractor = encoder_model_vit_update.feature_extractor_vit([3,5])  # I made this update
    ...
    if(other_args.functional != "prf-visualrois"):
        print("Higher visual architecture is being used")
        projector = encoder_model_vit_update.downproject_CLIP_split_linear_higher(num_higher_output=dataset.sizes)
    else:
        print("Early visual arch. is being used")
        projector = encoder_model_vit_update.downproject_CLIP_split_linear_early(num_early_output=dataset.sizes)
    # I made the above line update
    ...
    # _ = projector.to(rank)
    _ = projector.to(output_device)

    
    # ddp_projector = DDP(projector, find_unused_parameters=False, device_ids=[rank], gradient_as_bucket_view=True)
    ddp_projector = DDP(projector, find_unused_parameters=False, device_ids=[output_device], gradient_as_bucket_view=True)   #I made this upade from above line
    ...
    for epoch in range(start_epoch, other_args.epochs+1):
        ...
        if rank == 0:
            ...
            print("{}: Ending epoch {}, avg_training_loss {}, time {}, lr {}".format(other_args.exp_name, epoch, avg_loss, time() - old_time, new_lrate))
...
if __name__ == '__main__':
    ...
    functional = cur_args.functional
    region = cur_args.region
    ...
    exp_dir = os.path.join(cur_args.save_loc, exp_name_filled, functional, region)
