...
def test_net(rank, other_args):
    ...
    feature_extractor = encoder_model_vit.feature_extractor_vit()
    feature_extractor.to(rank)
    feature_extractor.eval()
    projector = encoder_model_vit.downproject_CLIP_split_linear(num_early_output=dataset.early_sizes, num_higher_output=dataset.higher_sizes)
    ...
    print("Saving to {}".format("/media/internal_8T/prashant/BrainDiVE/results/eval/{}.h5py".format(cur_args.exp_name)))

    # file = h5py.File("/ocean/projects/soc220007p/aluo/DiffusionInception/results/eval/{}.h5py".format(cur_args.exp_name), 'w')
    file = h5py.File("/media/internal_8T/prashant/BrainDiVE/results/eval/{}.h5py".format(cur_args.exp_name), 'w')
