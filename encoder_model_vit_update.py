...
class downproject_CLIP_split_linear_higher(torch.nn.Module):
    def __init__(self, num_higher_output=None):
        super().__init__()
        keys = sorted(num_higher_output.keys())
...
class downproject_CLIP_split_linear_early(torch.nn.Module):
    def __init__(self, num_early_output=None):
        super().__init__()
        keys = sorted(num_early_output.keys())
        for k_i in keys:
                self.add_module("final_{}_early".format(k_i), torch.nn.Linear(2048, num_early_output[k_i]))
