...
def main(argv):
    ...
    prefix = 'mar25_neurogen_code%d_finalized_imgs_present-'%code
    ...
    for sub in subs:
        for roi in roi_list:
            ...
            for cat_idx, c in enumerate(top_10_class):
                ...
                # folder = "/hdd_home/achin/projs/saten/xavierNew/AllCode/finalized_imgs_present/control/supp_0_class_0_rgb_0/code%d/Subject%d"%(code, sub)+"bg/class%s"%c+"/class_%d_sub%d_%s_seed0_trunc1/backup/"%(c, sub, roi)

                folder = "/hdd_home/achin/projs/neurogen/NeuroGen/mar26_img_code%d/S%d/ROI%d"%(code, sub, roi_dict[roi])
                ...
                for filename in os.listdir(folder):
                    if filename.startswith("C%04d"%c): 
                ...
                # img_list = [folder+s for s in imgs]
                img_list = imgs
                ...
                np.save(name + 'class_%d_'%c + roi + '_imgList.npy', np.asarray(img_list))
                ...
                classifier_score = np.zeros((len(img_list)//20, 20))
                for gen in tqdm(range(len(img_list)//20)):
