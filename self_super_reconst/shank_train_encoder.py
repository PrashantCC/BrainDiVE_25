    ...
    with SummaryWriter(comment='EncTrain') if FLAGS.sum_level > 0 else dummy_context_mgr() as sum_writer:
        ...
        with tqdm(desc='Epochs', total=FLAGS.n_epochs) if FLAGS.verbose < 1 else dummy_context_mgr() as pbar:
            ...
            for epoch in range(FLAGS.n_epochs):
                ...
                _, _, voxel_pearson_r_avg, collected_fmri, _ = \
                ...
                test_loss, meters_test, voxel_pearson_r_avg, collected_test, collected_test_data = \
                    train_test_regress(testloader, model, criterion, reg_loss_dict=reg_loss_dict, sum_writer=sum_writer)
                
                print("test_loss: ", test_loss, " corr: ", np.mean(voxel_pearson_r_avg))
                
                # consider correlation differently
                from scipy.stats import pearsonr as corr
#                 if FLAGS.sum_level > 3 and not optimizer :
                fmri_test_pred = torch.cat(collected_test_data['pred'],dim=0)
                fmri_test_act = torch.cat(collected_test_data['actual'],dim=0)
                fmri_correlation = np.zeros(fmri_test_pred.shape[1])
                for v in tqdm(range(fmri_test_pred.shape[1])):
                    fmri_correlation[v] = corr(fmri_test_pred[:,v], fmri_test_act[:,v])[0]
                print("Correlation using corr is: ", np.mean(fmri_correlation))
                ...
                if sum_writer:
                    ...
                    sum_writer.add_scalar('ValEnc/Corr',np.mean(fmri_correlation), epoch)
                ...
                is_best_corr = np.mean(voxel_pearson_r_avg) > best_corr
                best_corr = max(np.mean(voxel_pearson_r_avg), best_corr)
                if FLAGS.may_save:
                    if is_best_corr:
                        save_checkpoint({
                            'epoch': epoch + 1,
                            ...
                            'best_corr': best_corr,
                            'optimizer': optimizer.state_dict(),
                        }, is_best_corr=is_best_corr, filepath=get_checkpoint_out())
                    with open(os.path.join('checkpoints', 'val_results_sub{}_roi{}.txt'.format(FLAGS.sbj_num, FLAGS.roi)), 'a') as f:
                        f.write(f'epoch {epoch}, val_mean_corr: {np.mean(fmri_correlation)}, val_loss: {best_loss}, best_corr: {best_corr} \n') 
...
def train_test_regress(loader, model, criterion, optimizer=None, reg_loss_dict={},  sum_writer=None):
    ...
    collected_fmri_data = {'actual': [], 'pred': []}
        ...
        for batch_idx, (images, fmri_actual) in enumerate(loader):
            ...
            if FLAGS.sum_level > 3:
                if FLAGS.sum_level > 4 or not optimizer or FLAGS.pw_corr_win:
                    ...
                    collected_fmri_data['actual'].append(fmri_actual.data.cpu())
            ...
            if FLAGS.sum_level > 3:
                if FLAGS.sum_level > 4 or not optimizer or FLAGS.pw_corr_win:
                    ...
                    collected_fmri_data['pred'].append(fmri_pred.cpu().detach())
    ...
    return losses['total'].avg, \
           ...
           voxel_pearson_r.avg, collected_fmri,collected_fmri_data
