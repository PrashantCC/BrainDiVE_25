...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "joint_encoder_master")))

from encoder_dataloader_update import neural_loader, neural_loader_test
...
def main(argv):
    ...
    cprint1('=A= Summary level: {} =='.format(FLAGS.sum_level))

    # set_gpu()                     # Commented Achin 2704
    
    # output_device = int(FLAGS.gpu[0])
    # torch.cuda.set_device(output_device)
    def worker_init_fn(worker_id, myrank_info): np.random.seed(worker_id + myrank_info*100)
    
    other_args = argparse.Namespace(
        subject_id = [str(FLAGS.sbj_num)],
        neural_activity_path = FLAGS.neural_activity_path,
    ...
    train_dataset = neural_loader(other_args)
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, num_replicas=1, rank=0)
    ranked_worker_init = functools.partial(worker_init_fn, myrank_info=0)
    ...
    print(train_dataset.sizes[str(FLAGS.sbj_num)])
    
    test_dataset = neural_loader_test(other_args)
    test_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset, num_replicas=1, rank=0)
    ranked_worker_init = functools.partial(worker_init_fn, myrank_info=0)
    ...
    N_VOXELS = train_dataset.sizes[str(FLAGS.sbj_num)]
    ...
    # print(f"Moving model to device: {output_device}")
    ...
    # with open(f'/data6/shubham/PC/data/results_jointencoder_master_gaziv/runs/sub{str(FLAGS.sbj_num)}.txt', 'w') as f:
...
def train_test_regress(loader, model, criterion, optimizer=None, reg_loss_dict={},  sum_writer=None):
    ...
    with tqdm(desc=mode, total=len(loader)) if FLAGS.verbose > 0 else dummy_context_mgr() as bar:
        ...
        for batch_idx, data_stuff in enumerate(tqdm(loader, desc="Processing Batches")):
            # measure data loading time
            images = data_stuff["image_data"][:,0]
            fmri_actual = data_stuff["neural_data"]
