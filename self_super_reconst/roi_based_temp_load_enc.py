...
sys.path.append("/hdd_home/achin/projs/saten/xavierNew/AllCode/VGG_Class_Xdream/xdream/net_utils/")
...
def main(argv):
    ...
    # roi_list = ['V1v', 'FFA-2', 'PPA']


    roi_list = ['V1v', 'hV4']
    # enc_roi_list = ['V1v', 'hV4','FFA-2', 'PPA']
    enc_roi_list = ['V1v', 'hV4']
    # roi_list = ['FFA-2', 'PPA']

    # roi_list = ['V1v', "FFA-2", "PPA", "hV4", "V2v", "V3v", "VWFA-1", "OWFA", "mfs-words"]
   

    prefix = 'mar28_supp_rgb_no_class_constr-stark_code4_with_grad_2000_steps_1e-2_1e-2-'
    ...
    # pdb.set_trace()
    for enc_roi in enc_roi_list:
        for sub in subs:
            for roi in roi_list:

                print('ROI = ', roi)

                loc1 = f"/hdd_home/achin/projs/ss_recon/SelfSuperReconst/checkpoints_algo_30_epochs/sub{sub}_{enc_roi}_rgb_only.pth.tar"
                # loc2 = f"/hdd_home/achin/projs/ss_recon/SelfSuperReconst/checkpoints/sub{sub}_{roi2}_rgb_only.pth.tar"

                enc1, nvoxels1 = load_encoder(sub, enc_roi, loc1)
                ...
                for cat_idx, c in enumerate(top_10_class):
                    ...
                    "/hdd_home/achin/projs/saten/xavierNew/AllCode/finalized_imgs_present/optimal/code1/"
                    folder = "/hdd_home/achin/projs/saten/xavierNew/AllCode/transferred_Corrected_NonOpt_VGG_Class_Xdream/mar28_supp_rgb_no_class_constr-stark_code4_with_grad_2000_steps_1e-2_1e-2/Subject%d"%sub+"bg/class%s"%c+"/class_%d_sub%d_%s_seed0_trunc1/backup/"%(c, sub, roi)
                    
                    # folder = "/hdd_home/achin/projs/saten/xavierNew/AllCode/VGG_Class_Xdream/3009-maybeFin_Code1_MaxVit_Initnorm_awp_with_grad_1000_steps_1e-2_1e-2/Subject%d"%sub+"bg/class%s"%c+"/class_%d_sub%d_%s_seed0_trunc1/backup/"%(c, sub, roi)
                    ...
                    np.save(name + 'class_%d_'%c + roi + '_enc_roi_%s.npy'%enc_roi, roi_act)
