...
from resnet_load_classifier import load_resnet_18
...
def transform_image(img):
    ...
    resnet_im = img
    resnet_im = preprocess(resnet_im)
        ...
        for roi1 in roi_list_gen1:
            for roi2 in roi_list_gen2:
                ...
                if(suppress):
                    ...
                    if roi_name not in ["enhance_V1v_enhance_FFA-2", "enhance_V1v_enhance_PPA", "enhance_FFA-2_enhance_PPA"]:
                        continue
                ...
                for cat_idx, c in enumerate(top_10_class):
                    ...
                    folder = "/hdd_home/achin/projs/saten/xavierNew/AllCode/NSD_SOS_Xdream/0908_mixed_classifier_joint-soft/Subject%d"%sub+"bg/class%s"%c+"/"+roi_name+"/class_%d_sub%d_%s-%s_seed0_trunc1/backup/"%(c, sub, roi1, roi2)
                    ...
                    #save last gen imgs
                    np.save(name + 'class_%d_'%c + roi_name + '_imgList.npy', np.asarray(imgs_last_gen))
                    np.save(name + 'class_%d_'%c + roi1 + '.npy', roi1_act)
                    np.save(name + 'class_%d_'%c + roi2 + '.npy', roi2_act)
                    ...
                    np.save(name + 'class_%d_'%c + roi_name + '_classifier_score.npy', classifier_score)
